...
- The Gmail path is **deleted**, not left dormant: `gmail_oauth.py`, `token_store.py`, `smtp_sender.py`, the `/oauth/start` and `/oauth/callback` routes, and the `needs_authorization` contract in `send_book` are all gone, along with the `google-auth`/`google-auth-oauthlib` dependencies. The Cloudflare `/oauth/*` bypass rule is now removable.
